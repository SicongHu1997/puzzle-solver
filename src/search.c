...
int game_dijkstra_search(const game_info_t* info,
	...
	while (result == SEARCH_IN_PROGRESS){
		...
		int color = game_next_move_color(info, &n->state);
		for (int d=0; d<4; d++) {
			...
			if (game_can_move(info, &n->state, color, d)) {
				//Create child node
				tree_node_t* child = node_create(n, info, &n->state);
				...
				game_make_move(info, &child->state, color, d);
